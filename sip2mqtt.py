...
class SMAccountCallback(pj.AccountCallback):
    ...
    def on_reg_state(self):
        ...
        if self.account.info().reg_status == 200:
            broker.publish(args.mqtt_topic, payload="{\"verb\": \"registrationOK\", \"status\":\"" + str(self.account.info().reg_status)+ "\", \"reason\":\"" + str(self.account.info().reg_reason) + "\"}", qos=0, retain=True)
        else:
            broker.publish(args.mqtt_topic, payload="{\"verb\": \"registrationNOTOK\", \"status\":\"" + str(self.account.info().reg_status)+ "\", \"reason\":\"" + str(self.account.info().reg_reason) + "\"}", qos=0, retain=True)
            lib.destroy()
            main(sys.argv[1:])
    ...
    def on_incoming_call(self, call):
        ...
        #Lendy modify - call hangup, only notify Home Assistant
        call.answer(486)
